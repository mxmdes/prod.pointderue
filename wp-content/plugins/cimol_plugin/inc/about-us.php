<?php class cordon_about extends WP_Widget {

	// Defines the widget name
	public function __construct() {
		parent::__construct(false, $name = __('About Us Widget', 'cordon_plg') );
	}
	...
	function form($instance) {
		...
		<p>
	    <label for="<?php echo $this->get_field_id('image_uri'); ?>"><?php esc_html_e('About Us Image:', 'cordon_plg'); ?></label><br />
		...
		<p>
	    <input type="button" value="<?php esc_html_e( 'Upload Image', 'cordon_plg' ); ?>" class="button about_btn_upload" id="about_button"/>
	    </p><br />
		
		<p>
		<label for="<?php echo $this->get_field_id('textarea'); ?>"><?php esc_html_e('Textarea:', 'cordon_plg'); ?></label>
	...
	function widget($args, $instance) {
	   ...
	   echo '<div class="abtw-box" style="background-image:url('.$image_uri.');background-position: center;background-size: cover;">';
...
function register_about_us() {
	register_widget( 'cordon_about' );
}
add_action( 'widgets_init', 'register_about_us' );

// Load scripts in admin area
if( !function_exists ('cordon_about_scripts') ) :
	function cordon_about_scripts(){

	  wp_enqueue_media();
	  // Media upload script
	  wp_enqueue_script('cordon_about_upload', plugin_dir_url( __FILE__ ) . 'js/about-me.js');

	}
	add_action('admin_enqueue_scripts', 'cordon_about_scripts');
endif; // end cordon_about_scripts